...
const Options = () => {
  ...
  const loadAvailableBackups = async () => {
    ...
    try {
      const response = await chrome.runtime.sendMessage({ type: 'GET_AVAILABLE_BACKUPS' });
  ...
  const handleExportDatabase = async () => {
    ...
    try {
      const response = await chrome.runtime.sendMessage({ type: 'EXPORT_DATABASE' });
  ...
  const handleImportDatabase = async (file: File) => {
    ...
    try {
      ...
      if (!file.name.toLowerCase().endsWith('.sqlite') && !file.name.toLowerCase().endsWith('.db')) {
        setImportStatus("Please select a valid SQLite database file (.sqlite or .db)");
      ...
      if (response.success) {
        setImportStatus("Database imported successfully! Refreshing backups...");
      ...
      <div style={{ 
        ...
        <section style={{ marginBottom: 40 }}>
          ...
          <div style={{ 
            ...
            <ol style={{ 
              ...
              <li style={{ marginBottom: 4 }}>Choose "Fine-grained personal access token"</li>
              <li style={{ marginBottom: 4 }}>Set expiration and description (e.g., "Kuery Extension")</li>
              <li style={{ marginBottom: 4 }}>In <strong>"Account permissions"</strong> section, enable <strong>"Models - Read-only"</strong></li>
          ...
          {tokenTestStatus && (
            <div style={{ 
              marginTop: 16,
              padding: '8px 12px', 
              backgroundColor: tokenTestStatus.includes("✅") ? '#f0fdf4' : tokenTestStatus.includes("❌") ? '#fef2f2' : '#f0f9ff',
              color: tokenTestStatus.includes("✅") ? '#16a34a' : tokenTestStatus.includes("❌") ? '#dc2626' : '#2563eb',
              borderRadius: 6,
              fontSize: '14px',
        ...
        <section style={{ marginBottom: 40 }}>
          ...
          <p style={{ 
            ...
            Import databases from other devices or export your current database and backups as SQLite files.
          </p>
          ...
          {(exportStatus || importStatus) && (
            <div style={{ marginBottom: 16 }}>
              {exportStatus && (
                <div style={{ 
                  padding: '8px 12px', 
                  backgroundColor: exportStatus.includes("failed") ? '#fef2f2' : '#f0f9ff',
                  color: exportStatus.includes("failed") ? '#dc2626' : '#2563eb',
                  borderRadius: 6,
              ...
              {importStatus && (
                <div style={{ 
                  padding: '8px 12px', 
                  backgroundColor: importStatus.includes("failed") ? '#fef2f2' : '#f0fdf4',
                  color: importStatus.includes("failed") ? '#dc2626' : '#16a34a',
                  borderRadius: 6,
                  fontSize: '14px',
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ fontSize: '12px', color: '#6b7280', marginBottom: 8 }}>
              Select a .sqlite or .db file to import. <strong>Warning:</strong> This will replace your current database!
            </div>
            <div style={{ 
              fontSize: '11px', 
              color: '#dc2626', 
              ...
              ⚠️ A backup of your current database will be created automatically before import.
            </div>
          </div>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
              ...
              <button 
                ...
                onMouseEnter={(e) => {
                  if (!isLoadingBackups) e.target.style.backgroundColor = '#4b5563';
                }}
                onMouseLeave={(e) => {
            ...
            {availableBackups.length > 0 ? (
              <div style={{ 
                ...
                {availableBackups.map((backup, index) => (
                  <div 
                    ...
                    style={{ 
                      padding: '12px 16px', 
                      borderBottom: index < availableBackups.length - 1 ? '1px solid #e2e8f0' : 'none',
                      display: 'flex',
                  ...
                  >
                    ...
                    <button 
                      onClick={() => handleExportBackup(backup.key, backup.date)}
                      style={{ 
                        padding: '6px 12px', 
              ...
              <div style={{ 
                ...
                {isLoadingBackups ? 'Loading backups...' : 'No database backups found'}
              </div>
            )}
          </div>
        </section>
