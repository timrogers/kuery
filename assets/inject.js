(function() {
  ...
  window.fetch = function(...args) {
    ...
    return originalFetch.apply(this, args).then(response => {
      console.log('Kuery: Fetch response received for:', resourceStr, 'Status:', response.status);
      ...
      if (isReady && shouldInterceptRequest(resourceStr, config)) {
        ...
        console.log('Kuery: This request will not be intercepted. isReady:', isReady, 'shouldIntercept:', shouldInterceptRequest(resourceStr, config));
      }
      return response;
    });
  };
  ...
  XMLHttpRequest.prototype.send = function(data) {
    const self = this;
    console.log('Kuery: XHR send intercepted:', self._url, 'Method:', self._method);
    
    if (isReady && shouldInterceptRequest(self._url, { method: self._method, body: data })) {
      ...
      this.addEventListener('load', () => {
        ...
        if (this.status >= 200 && this.status < 300) {
          ...
          interceptKustoRequest(self._url, { method: self._method, body: data }, mockResponse);
        }
      });
    } else {
      console.log('Kuery: This XHR request will not be intercepted. isReady:', isReady, 'shouldIntercept:', shouldInterceptRequest(self._url, { method: self._method, body: data }));
    }
  ...
  function shouldInterceptRequest(url, config) {
    ...
    return kustoPatterns.some(pattern => pattern.test(url)) &&
  ...
  async function interceptKustoRequest(url, config, response) {
    ...
    try {
      ...
      if (requestBody) {
        queryText = requestBody.csl || requestBody.query || requestBody.Query || 
      ...
      try {
        ...
        console.log('Kuery: Raw response preview:', responseText.substring(0, 500));
      ...
      if (responseData) {
        console.log('Kuery: Full response data structure:', JSON.stringify(responseData, null, 2));
        
        // Check if response is an array (v2 API format)
        if (Array.isArray(responseData)) {
          console.log('Kuery: Response is array with', responseData.length, 'elements');
          
          // Look for PrimaryResult table in the array
          const primaryResultTable = responseData.find(item => 
            item && item.TableKind === 'PrimaryResult' && item.Rows
          );
          
          if (primaryResultTable) {
            console.log('Kuery: Found PrimaryResult table with', primaryResultTable.Rows.length, 'rows');
            hasResults = primaryResultTable.Rows.length > 0;
            resultCount = primaryResultTable.Rows.length;
          } else {
            // Look for any DataTable with rows
            const dataTables = responseData.filter(item => 
              item && item.FrameType === 'DataTable' && item.Rows
            );
            
            if (dataTables.length > 0) {
              const totalRows = dataTables.reduce((sum, table) => sum + (table.Rows ? table.Rows.length : 0), 0);
              console.log('Kuery: Found', dataTables.length, 'data tables with total', totalRows, 'rows');
        ...
        // Handle object-based responses (v1 API format)
          ...
          if (responseData.Tables) {
            ...
            console.log('Kuery: Found Tables array, length:', responseData.Tables.length);
            
            // Check if tables have rows
            if (responseData.Tables.length > 0 && responseData.Tables[0].Rows) {
              console.log('Kuery: First table has', responseData.Tables[0].Rows.length, 'rows');
            ...
            console.log('Kuery: PrimaryResult keys:', Object.keys(primaryResult));
            if (primaryResult.Tables) {
              hasResults = primaryResult.Tables.length > 0;
              resultCount = primaryResult.Tables.length;
              console.log('Kuery: Found PrimaryResult.Tables, length:', primaryResult.Tables.length);
            }
            ...
            console.log('Kuery: No recognized result structure found in response');
            ...
            for (const [key, value] of Object.entries(responseData)) {
              if (Array.isArray(value)) {
                console.log('Kuery: Found array property', key, 'with length', value.length);
                if (value.length > 0) {
      ...
      if (queryText && queryText.trim()) {
        console.log('Kuery: Intercepted query with hasResults:', hasResults, 'resultCount:', resultCount);
        window.postMessage({
